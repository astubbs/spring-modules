 ...
 * Usually interface implementations will extends this class.
 * 
 * @author Costin Leau
 * 
 */
public abstract class AbstractSessionHolderProviderManager implements SessionHolderProviderManager {
	...
	public SessionHolderProvider getSessionProvider(final Repository repository) {
		...
		final String key = repository.getDescriptor(Repository.REP_NAME_DESC);
		final List providers = getProviders();

		// search the provider
		for (int i = 0; i < providers.size(); i++) {
			final SessionHolderProvider provider = (SessionHolderProvider) providers.get(i);
