...
public abstract class JcrUtils {
	...
	/**
	 * Class used for escaping XML names which contain restricted character as defined in the JCR spec version 1.0
	 * Section 6.2.5.2. (i.e. "/", ":", "[", "]", "*", "'", """, "|", and all whitespace characters other than " "). The
	 * escaping schema used is described in the same document in paragraphs 6.4.3 and 6.4.4.
	 * 
	 * The class was initially imported from the <a href="http://svn.apache.org/viewcvs.cgi/incubator/jackrabbit/trunk/jackrabbit/src/main/java/org/apache/jackrabbit/util/ISO9075.java?view=markup"
	 * >Jackrabbit project</a>.
	 * 
	 * Implements the encode and decode routines as specified for XML name to SQL identifier conversion in ISO
	 * 9075-14:2003.<br/>
	 * If a character <code>c</code> is not valid at a certain position in an XML 1.0 NCName it is encoded in the form:
	 * '_x' + hexValueOf(c) + '_'
	 * <p/>
	 */
	protected static class ISO9075 {
		...
		public String decode(final String name) {
			...
			final Matcher m = ENCODE_PATTERN.matcher(name);
			while (m.find()) {
				m
						.appendReplacement(decoded, Character.toString((char) Integer.parseInt(m.group()
								.substring(2, 6), 16)));
			}
		 ...
		 * <code>"_x" + hex value of c + "_"</code>. Where the hex value has four digits if the character with possibly
		 * leading zeros.
		 * <p/>
		 ...
		 */
		private void encode(final char c, final StringBuffer b) {
			b.append("_x");
		 ...
		 */
		private boolean needsEscaping(final String name, final int location) throws ArrayIndexOutOfBoundsException {
