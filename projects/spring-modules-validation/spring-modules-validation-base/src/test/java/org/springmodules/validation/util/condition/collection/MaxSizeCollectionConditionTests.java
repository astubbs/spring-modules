...
/**
 * Tests for {@link MaxSizeCollectionCondition}.
 *
 * @author Uri Boness
 */
public class MaxSizeCollectionConditionTests extends AbstractCollectionConditionTests {

    protected AbstractCollectionCondition createCondition() {
        return new MaxSizeCollectionCondition(3);
    }

    public void testConstructor_WithNegativeMaxSize() throws Exception {
        try {
            new MaxSizeCollectionCondition(-1);
            fail("An IllegalArgumentException must be thrown if given max size is nagative");
    ...
    public void testCheck_WithBiggerCollection() throws Exception {
        Collection collection = new FluentList().addObject("1").addObject("2").addObject("3").addObject("4");
