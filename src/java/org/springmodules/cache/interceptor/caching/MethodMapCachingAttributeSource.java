 ...
 */
public class MethodMapCachingAttributeSource implements CachingAttributeSource {

  /**
   * Message logger.
   */
  private static Log logger = LogFactory
      .getLog(MethodMapCachingAttributeSource.class);

  /**
   * Map containing instances of <code>{@link Cached}</code>. The key of each
   * entry is instance of <code>{@link Method}</code> to attach the attribute
   * to.
   */
   ...
   */
  private void addCachingAttribute(Class targetClass, String targetMethodName,
    ...
    while (matchingMethodsIterator.hasNext()) {
      ...
      if (registeredMethodName == null
        ...
        this.addCachingAttribute(method, cachingAttribute);
   ...
   * @param cachingAttribute
   *          the caching attribute used as value of the new entry.
   */
  private void addCachingAttribute(Method method, Cached cachingAttribute) {
    logger.info("Adding method [" + method + "] with caching attribute ["
        + cachingAttribute + "]");
    this.attributeMap.put(method, cachingAttribute);
  }
   ...
   *           be found.
   * 
   * @see #addCachingAttribute(Class, String, Cached)
   */
  public final void addCachingAttribute(String fullyQualifiedMethodName,
    ...
    try {
      ...
      this.addCachingAttribute(targetClass, methodName, cachingAttribute);
  ...
  public final Cached getCachingAttribute(Method method, Class targetClass) {
    return (Cached) this.attributeMap.get(method);
  }
