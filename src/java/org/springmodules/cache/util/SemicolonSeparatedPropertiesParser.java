...
public abstract class SemicolonSeparatedPropertiesParser {
  ...
  public static Properties parseProperties(String text)
    ...
    if (text.endsWith(PROPERTY_DELIMITER)) {
      // remove ';' at the end of the text (if applicable)
      text = text.substring(0, text.length() - PROPERTY_DELIMITER.length());
    ...
    String[] propertiesAsText = StringUtils.delimitedListToStringArray(text,
        PROPERTY_DELIMITER);
